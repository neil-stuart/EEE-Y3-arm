...
x_scale = Scale(root, from_=-1.0, to=1.0, resolution=0.25, orient=HORIZONTAL, label="X Position", command=on_slider_change)
#x_scale.set(0.3)  # Default value
x_scale.pack(fill='x')  # Ensure it fills the space along the x-axis

y_scale = Scale(root, from_=-1.0, to=1.0, resolution=0.25, orient=HORIZONTAL, label="Y Position", command=on_slider_change)
#y_scale.set(0.3)  # Default value
y_scale.pack(fill='x')  # Same for Y

z_scale = Scale(root, from_=-1.0, to=1.0, resolution=0.25, orient=HORIZONTAL, label="Z Position", command=on_slider_change)
